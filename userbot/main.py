...
ALIVE_MSG = [
    "`Userbotunuz çalışıyor ve sana bişey demek istiyor.. Seni seviyorum` **{ultrosahip}** ❤️ \n Bot Versiyonu: {ultro} ",
    "🎆 `Endişelenme! Seni yanlız bırakmam.` **{ultrosahip}**, `UltroUserbot çalışıyor.` \n Bot Versiyonu: {ultro} ",
    "`⛈️ Elimden gelenin en iyisini yapmaya hazırım`, **{ultrosahip}** \n Bot Versiyonu: {ultro} ",
    "✨ `UltroUserBot sahibinin emirlerine hazır...` \n Bot Versiyonu: {ultro} ",
    "`Şuan en gelişmiş userbotun düzenlediği mesajı okuyor olmalısın` **{ultrosahip}**. \n Bot Versiyonu: {ultro} ",
...
CV_MSG = [
    "**{DEFAULT_NAME}** `Fazla Bi Bilgi Ayarlamamış Ama Şunu Biliyorum Kendisi Baya Zevkli Birisi Çünkü Ultro Userbot Kullanıyor.` 😁",
...
try:
    bot.start()
    idim = bot.get_me().id
    ultrobl = requests.get('https://raw.githubusercontent.com/erdewbey/datas/master/blacklist.json').json()
    if idim in ultrobl:
