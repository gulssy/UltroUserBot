    ...
    if ((len(dil.file.name.split(".")) >= 2) and (dil.file.name.split(".")[1] == "ultrojson")):
        if path.isfile(f"./userbot/language/{dil.file.name}"):
            try:
                ...
                if path.isfile("./userbot/language/DEFAULT.ultrojson"):
                    LOGS.warn("Varsayılan dil dosyası kullanılıyor...")
                    LANGUAGE_JSON = loads(open(f"./userbot/language/DEFAULT.ultrojson", "r").read())
                else:
...
if LANGUAGE_JSON == None:
    if path.isfile(f"./userbot/language/{LANGUAGE}.ultrojson"):
        try:
            LANGUAGE_JSON = loads(open(f"./userbot/language/{LANGUAGE}.ultrojson", "r").read())
        except JSONDecodeError:
